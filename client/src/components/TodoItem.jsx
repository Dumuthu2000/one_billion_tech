...
const TodoItem = ({ todo, onEdit, onDelete }) => {
  ...
  return (
    <div
      className={`rounded-xl shadow-sm p-6 transition-all duration-300 border border-gray-100 ${getPriorityStyles()} hover:shadow-lg`}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex-1 space-y-2">
          ...
          <p
            className={`text-sm ${isCompleted ? 'text-gray-400' : 'text-gray-600'} line-clamp-2`}
          >
            {description}
          </p>
        </div>
